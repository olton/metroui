    ...
    Metro.Component('streamer', {
        ...
        build: function(){
            ...
            if (data.actions !== undefined) {
                ...
                $.each(data.actions, function(){
                    ...
                    if (this.onclick) button.on(Metro.events.click, ()=> {
            ...
            if (data.streams !== undefined) {
                $.each(data.streams, function(stream_index, stream_item){
                    ...
                    const stream_events = $("<div>").addClass("stream-events")
                    ...
                    if (stream_item.events !== undefined) {
                        $.each(stream_item.events, function(event_index, event_item){
                            ...
                            const custom_html_close = event_item.custom_close !== undefined ? event_item.custom_close : "";
                            let event;
                            ...
                            if (Metro.utils.isNull(event_item.html)) {
                                ...
                                if (o.closed === false && (element.attr("id") === StreamerIDS_i && StreamerIDS_a.indexOf(sid) !== -1) || event_item.selected === true || Number.parseInt(event_item.selected) === 1) {
                                    event.addClass("selected");
                                }

                                if (o.closed === true || event_item.closed === true || Number.parseInt(event_item.closed) === 1) {
                                    _icon = event_item.closedIcon !== undefined ? Metro.utils.isTag(event_item.closedIcon) ? event_item.closedIcon : `<span>${event_item.closedIcon}</span>` : Metro.utils.isTag(o.defaultClosedIcon) ? o.defaultClosedIcon : `<span>${o.defaultClosedIcon}</span>`;
                                    ...
                                    _icon = event_item.openIcon !== undefined ? Metro.utils.isTag(event_item.openIcon) ? event_item.openIcon : `<span>${event_item.openIcon}</span>` : Metro.utils.isTag(o.defaultOpenIcon) ? o.defaultOpenIcon : `<span>${o.defaultOpenIcon}</span>`;
                ...
                $.each(['before', 'after'], (_, global_item)=> {
                    if (data.global[global_item] !== undefined) {
                        $.each(data.global[global_item], function(_, event_item){
                            const group = $("<div>").addClass("event-group").addClass(`size-${event_item.size}${["half", "one-third"].includes(event_item.size) ? "" : "x"}`);
        ...
        _createEvents: function(){
            ...
            if (o.wheel === true) {
                ...
                element.find(".events-area").off("mouseenter").on("mouseenter", () => {
                    disableScroll();
                });

                element.find(".events-area").off("mouseleave").on("mouseleave", () => {
                    enableScroll();
                });
            }

            element.find(".events-area").last().off("scroll").on("scroll", ()=> {
                that._fireScroll();
            });
        ...
        selectEvent: function(event, state = true){
            ...
            if (state === true) _event.addClass("selected"); else _event.removeClass("selected");
